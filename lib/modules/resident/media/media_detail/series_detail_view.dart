...
class _SeriesDetailViewState extends State<SeriesDetailView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      child: Column(
        ...
        SizedBox(
          ...
          child: FocusCluster(
            ...
            itemBuilder: (context, index) {
              ...
              final isSelected = id == _selectedSeasonId;

              return FocusIdentity(
                id: 'season_$id',
                alignment: FocusAlignment.top,
                onPressed: () {
                  if (id != null && id != _selectedSeasonId) {
                    setState(() {
        ...
        // Episodes List
        _buildEpisodes(material, grid),
      ],
    ),
    );
  }

  Widget _buildEpisodes(ResolvedThemeMaterial material, GridContext grid) {
  ...
  Widget _buildGridEpisodes(ResolvedThemeMaterial material, GridContext grid) {
    final int totalPages = (_episodes.length / 50).ceil();
    final int startIdx = _currentEpisodePage * 50;
    final int endIdx = (startIdx + 50).clamp(0, _episodes.length);
    final pageEpisodes = _episodes.sublist(startIdx, endIdx);
        ...
        SizedBox(
          ...
          child: FocusCluster(
            ...
            itemBuilder: (context, index) {
              ...
              final endRange = ((index + 1) * 50).clamp(1, _episodes.length);
                      ...
                      builder: (context, hasFocus) {
                        if (hasFocus && _focusedEpisodeTitle != displayTitle) {
                          WidgetsBinding.instance.addPostFrameCallback((_) {
                            if (mounted) setState(() { _focusedEpisodeTitle = displayTitle; });
                          });
                        }
