...
class MediaCallback {
  ...
  static Future<void> onMediaPosterTap(BuildContext context, MediaItem item, String? currentExpandedId) async {
    ...
    if (item.jellyfinType == 'BoxSet') {
      if (currentExpandedId == item.id) {
        ...
        FocusAPI.dispatchAction(
          ...
          'mediaExpand_${item.id}',
        );
      }
    } else {
      _isNavigating = true;
      FocusAPI.setCursorWaiting(true);

      await MediaService.instance.preloadDetailData(item);

      FocusAPI.setCursorWaiting(false);
      _isNavigating = false;
  ...
  static Future<void> onBoxSetChildTap(BuildContext context, MediaItem childItem, String boxSetId) async {
    if (_isNavigating) return;
    
    _isNavigating = true;
    FocusAPI.setCursorWaiting(true);

    await MediaService.instance.preloadDetailData(childItem);
